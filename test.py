...
def test_example(c):
    ...
    try:
        ...
        ans_max = list(filter(lambda x:  re.match(r'^max', x, re.I), ans))
        ans = list(filter(lambda x: not re.match(r'^max', x, re.I), ans))
        ...
        for line in out:
            while line[-1] == '\n' or line[-1] == ' ' or line[-1] == '\r':
                line = line[:-1]
        out_max = list(filter(lambda x:  re.match(r'^max', x, re.I), out))
        out = list(filter(lambda x: not re.match(r'^max', x, re.I), out))
